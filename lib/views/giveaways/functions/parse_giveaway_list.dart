...
GiveawayListModel parseGiveawayListElement(dom.Element element,
    [String? username]) {
  dom.Document item = parse(element.innerHtml);
  dom.Element name = item.getElementsByClassName('giveaway__heading__name')[0];
  List<dom.Element> heading =
      item.getElementsByClassName('giveaway__heading__thin');
  ...
  return GiveawayListModel(
      ...
      entered: element.innerHtml.contains('giveaway__row-inner-wrap is-faded')
          ? true
          : false,
      ...
      copies: points != null
          ...
          : null,
      points: points != null
          ? int.parse(points.substring(1, points.length - 2))
          : null,
      level: lvl.isEmpty
          ? 0
          : int.parse(lvl[0].text.substring(6, lvl[0].text.length - 1)),
      inviteOnly: item
          .getElementsByClassName('giveaway__column--invite-only')
          .isNotEmpty,
      group: item.getElementsByClassName('giveaway__column--group').isNotEmpty,
      whitelist:
          item.getElementsByClassName('giveaway__column--whitelist').isNotEmpty,
      region: item
          .getElementsByClassName('giveaway__column--region-restricted')
          .isNotEmpty,
